...
while (1)
      {
        ...
        if (last_min != minute_read || last_sec != second_read ){   
            minute_counter = minute_read;
            second_counter = second_read;
            last_min = minute_read;
            last_sec = second_read;
        } 
        
        sprintf(time , "%d : %d " , minute_counter , second_counter);
        lcd_gotoxy(0,0);
        lcd_puts(time);
