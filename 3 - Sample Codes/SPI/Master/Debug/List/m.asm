...
; 0000 0026     SPDR = data;                       //Load data into the buffer
	ST   -Y,R26
;	data -> Y+0
	LD   R30,Y
	OUT  0xF,R30
; 0000 0027     while(!(SPSR & (1<<SPIF) ));       //Wait until transmission complete
_0x3:
	SBIS 0xE,7
	RJMP _0x3
; 0000 0028     return(SPDR);                      //Return received data
	IN   R30,0xF
	RJMP _0x2040001
; 0000 0029 }
; .FEND
...
; 0000 002E 
; 0000 002F //Data to send
; 0000 0030 char data[6] = {'h','e','l','l','o','!'};
; 0000 0031 //Transmittion counter
...
; 0000 003C // Function: Bit7=Out Bit6=In Bit5=Out Bit4=Out Bit3=In Bit2=In Bit1=In Bit0=In
; 0000 003D DDRB=(1<<DDB7) | (0<<DDB6) | (1<<DDB5) | (1<<DDB4) | (0<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(176)
	OUT  0x17,R30
; 0000 003E // State: Bit7=0 Bit6=T Bit5=0 Bit4=0 Bit3=T Bit2=T Bit1=T Bit0=T
...
; 0000 004E // SPI Type: Master
; 0000 004F // SPI Clock Rate: 62.500 kHz
; 0000 0050 // SPI Clock Phase: Cycle Start
; 0000 0051 // SPI Clock Polarity: Low
; 0000 0052 // SPI Data Order: MSB First
; 0000 0053 SPCR=(0<<SPIE) | (1<<SPE) | (0<<DORD) | (1<<MSTR) | (0<<CPOL) | (0<<CPHA) | (1<<SPR1) | (1<<SPR0);
	LDI  R30,LOW(83)
	OUT  0xD,R30
; 0000 0054 SPSR=(0<<SPI2X);
	LDI  R30,LOW(0)
	OUT  0xE,R30
; 0000 0055 
...
; 0000 0066             if(counter == 6){
	LDI  R30,LOW(6)
	LDI  R31,HIGH(6)
	CP   R30,R16
	CPC  R31,R17
	BRNE _0x9
; 0000 0067                 lcd_puts("Completed");
	__POINTW2MN _0xA,0
	RCALL _lcd_puts
; 0000 0068                 counter = -1;// Finish the process
	__GETWRN 16,17,-1
; 0000 0069             }
; 0000 006A             if(counter != -1){
_0x9:
	LDI  R30,LOW(65535)
	LDI  R31,HIGH(65535)
	CP   R30,R16
	CPC  R31,R17
	BREQ _0xB
; 0000 006B                 spi_tranceiver(data[counter]);
	MOVW R26,R28
	ADD  R26,R16
	ADC  R27,R17
	LD   R26,X
...
; 0000 006C                 counter ++;
	__ADDWRN 16,17,1
; 0000 006D                 delay_ms(50); // let slave process the data in the INT
	LDI  R26,LOW(50)
	LDI  R27,0
	CALL _delay_ms
; 0000 006E             }
