...
_0x0:
	.DB  0x26,0x0,0x7C,0x0,0x2D,0x0,0x2B,0x0
	.DB  0x25,0x64,0x25,0x63,0x25,0x64,0x20,0x3D
	.DB  0x20,0x25,0x64,0x0,0x31,0x0,0x30,0x0
_0x2000003:
	.DB  0x80,0xC0
...
; 0000 002B     a = PINB.0 + PINB.1 * 2 + PINB.2 * 4 + PINB.3 * 8 + PINB.4 * 16;
	LDI  R30,0
	SBIC 0x16,0
	LDI  R30,1
	MOV  R22,R30
	...
	MOV  R4,R30
	CLR  R5
; 0000 002C     b = PINA.3 + PINA.4 * 2 + PINA.5 * 4 + PINA.6 * 8 + PINA.7 * 16;
	LDI  R30,0
	SBIC 0x19,3
	LDI  R30,1
	MOV  R22,R30
	...
	MOV  R6,R30
	CLR  R7
; 0000 002D     if(PINB.6 == 1) {res = a & b; sprintf(op,"&");}
	SBIS 0x16,6
	RJMP _0x3
	MOVW R30,R6
	AND  R30,R4
	...
	__POINTW1FN _0x0,0
	RJMP _0x11
; 0000 002E     else if(PINB.7 == 1) {res = a | b; sprintf(op,"|");}
_0x3:
	SBIS 0x16,7
	RJMP _0x5
	MOVW R30,R6
	...
	__POINTW1FN _0x0,2
	RJMP _0x11
; 0000 002F     else if(PINA.1 == 1) {res = a - b; sprintf(op,"-");}
_0x5:
	SBIS 0x19,1
	RJMP _0x7
	MOVW R30,R4
...
_0x11:
	...
	CALL _sprintf
	ADIW R28,4
; 0000 0030     if(PINA.0 == 1) {res = a + b; sprintf(op,"+");}
_0x7:
	SBIS 0x19,0
	RJMP _0x8
	MOVW R30,R6
	...
	CALL _sprintf
	ADIW R28,4
; 0000 0031     sprintf(str,"%d%c%d = %d", a, op[0], b, res);
_0x8:
	LDI  R30,LOW(_str)
	LDI  R31,HIGH(_str)
	ST   -Y,R31
...
; 0000 0036     lcd_gotoxy(0,0);
	...
	LDI  R26,LOW(0)
	RCALL _lcd_gotoxy
; 0000 0037     //printBin(a);
; 0000 0038     //lcd_puts(op);
; 0000 0039     //printBin(b);
; 0000 003A     //lcd_puts("=");
; 0000 003B     printBin(res);
	MOVW R26,R8
	RCALL _printBin
; 0000 003C }
	LD   R30,Y+
...
; 0000 0089 // INT0: On
; 0000 008A // INT0 Mode: Rising Edge
; 0000 008B // INT1: Off
; 0000 008C // INT2: Off
; 0000 008D GICR|=(0<<INT1) | (1<<INT0) | (0<<INT2);
	...
	ORI  R30,0x40
	OUT  0x3B,R30
; 0000 008E MCUCR=(0<<ISC11) | (0<<ISC10) | (1<<ISC01) | (1<<ISC00);
	LDI  R30,LOW(3)
	OUT  0x35,R30
...
;
;void printBin(int n){
; 0000 00C2 void printBin(int n){
_printBin:
; .FSTART _printBin
; 0000 00C3     if (n > 1) printBin(n/2);
	ST   -Y,R27
	ST   -Y,R26
;	n -> Y+0
	LD   R26,Y
